...
func ValidateStringArrayLength(stringList []string, keyName string, minimum, maximum, stringLengthMinimum, stringLengthMaximum int) ValidateFunc {

	return func() error {

		length := len(stringList)
		if maximum > 0 && length > maximum {
			return fmt.Errorf("%q count is too large", keyName)
		}
		if minimum > 0 && length < minimum {
			return fmt.Errorf("%q count is too small", keyName)
		}

		for _, str := range stringList {

			err := ValidLength(str, keyName, stringLengthMinimum, stringLengthMaximum)
			if err != nil {
				return err
			}
		}
...
func ValidateUint64PositiveIntegerPointer(value *uint64, keyName string) ValidateFunc {
	...
	return func() error {
		...
		return ValidateUint64PositiveInteger(*value, keyName)()
	}
}

func ValidateUint8Options(value uint8, keyName string, options []uint8) ValidateFunc {
...
func ValidateUint8PointerOptions(value *uint8, keyName string, options []uint8) ValidateFunc {
